...
namespace GameArki.TripodCamera.API {
    ...
    public interface IFollowLookSetterAPI {

        /// <summary>
        /// 初始化指定ID相机的 #跟随点 #跟随位置偏移
        /// </summary>
        void Follow_SetInit(Transform target, in Vector3 offset, int id);

        /// <summary>
        /// 初始化指定ID相机的 #跟随点 #跟随位置偏移 #缓动函数 #缓动时间
        /// </summary>
        void Follow_SetInit(Transform target, in Vector3 offset, EasingType easingType, float easingTime, int id);

        /// <summary>
        /// 初始化指定ID相机的 #跟随点 #跟随位置偏移 #缓动函数(水平方向) #缓动时间(水平方向) #缓动函数(垂直方向) #缓动时间(垂直方向)
        /// </summary>
        void Follow_SetInit(Transform target,
                            in Vector3 offset,
        ...
        /// 设置指定ID相机的 #跟随位置偏移
        /// </summary>
        void Follow_ChangeOffset(in Vector3 offset, int id);
